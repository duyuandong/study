...
public class DateUtils {
  ...
  /**
   * @author: duyd@segimail.com @Description: addHours @Param: [date, hour] @Return:
   *     java.util.Date @Date: 2021/9/16 15:49
   */
  ...
  public static String parseStrCycleToYYMMStrDate(String strCycle) {
    if (strCycle == null || strCycle.length() < 6) return "";
   ...
   * 20220218转2022-02-18
   *
   * @author: duyd@segimail.com @Description: dateYYMMDDFormatYY_MM_DD @Param: [dateStr] @Return:
   *     java.lang.String @Date: 2022/2/18 11:38
   */
  public static String dateYYMMDDFormatYY_MM_DD(String dateStr) {
    if (StringUtils.isBlank(dateStr) && dateStr.length() != 8) {
      return "";
    }
    return new StringBuilder(dateStr).insert(4, "-").insert(7, "-").toString();
  }

  private static boolean isNumericForStartAndEnd(String str) {
    Pattern pattern =
        Pattern.compile(
            "((\\d{4}|\\d{2})-((0?([1-9]))|(1[0|1|2]))-((0?[1-9])|([1|2]([0-9]))|(3[0|1])))");
  ...
  public static final int daysBetween(Date early, Date late) {
    ...
    // 得到两个日期相差的天数
    int days =
        ((int) (caled.getTime().getTime() / 1000) - (int) (calst.getTime().getTime() / 1000))
            / 3600
            / 24;
  ...
  public static final int minutesBetween(Date early, Date late) {
    ...
    // 得到两个日期相差的分钟数
    int minutes =
        ((int) (caled.getTime().getTime() / 1000) - (int) (calst.getTime().getTime() / 1000)) / 60;
  ...
  public static int getDateSpace(String fromDate, String toDate) {
    ...
    result =
        (toDateCd.get(Calendar.YEAR) - fromDateCd.get(Calendar.YEAR)) * 12
            + (toDateCd.get(Calendar.MONTH) - fromDateCd.get(Calendar.MONTH));
  ...
  public static void main(String[] arg) {
    ...
    numberFormat.setMaximumFractionDigits(0); // 设置精确度
    String result = numberFormat.format(Float.valueOf("3.3") / Float.valueOf("100") * 100);
    ...
    String result2 = numberFormat2.format((float) num1 / (float) num2 * 100);

    System.out.println("num1和num2的百分比为:" + result2 + "%");

    System.out.println(getWorkdayCountOfMonth(new Date()));
    System.out.println(getWorkdayCountOfMonth(parseStringToDate("2019-03", "yyyy-MM")));
    System.out.println(getHolidayCountOfMonth(new Date()));
    System.out.println(getHolidayCountOfMonth(parseStringToDate("2019-03", "yyyy-MM")));

    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
    try {
      System.out.println(getDaysOfMonthNow(sdf.parse("2019-05-02")));
    } catch (ParseException e) {
      e.printStackTrace();
    }

    System.out.println(getWorkdayCountOfMonthNow(new Date()));
    System.out.println(getHolidayCountOfMonthNow(new Date()));

    System.out.println("两个日期相差月份" + getDateSpace("2019-01-01 12:12:12", "2019-01-02 12:12:11"));

    System.out.println("2019-01-31 12:59:59".substring(0, 10));
    System.out.println("2019-01-31 12:59:59".substring(0, 11));

    System.out.println(lastDayOfLastMonth(new Date()));

    System.out.println(addDateHours(new Date(), -2));
    System.out.println("------------------------------------");
    System.out.println(
        DateUtils.formatDateToStringYYMMDDHHmmss(
            DateUtils.addDateDay(
                DateUtils.parseStringToDate("2021-12-31 00:00:00", "yyyy-MM-dd HH:mm:ss"), 1)));

    System.out.println(addDateMonth(parseStringToDate("2022-09-16", FORMAT_YYYY_MM_DD), 1));

    System.out.println(dateYYMMDDFormatYY_MM_DD("20220218"));

    System.out.println(
        minutesBetween(
            DateUtils.parseStringToDateYYMMDDHHmmss("2021-11-22 11:53:01"),
            DateUtils.parseStringToDateYYMMDDHHmmss("2022-03-23 16:40:21")));

    System.out.println(formatDateToStringYYMMDDHHmmss(addMonthNumbersOfDate(new Date(), 2)));

    Date startDate = DateUtils.parseStringToDateYYMMDDHHmmss("2022-03-26 17:03:34");
    Date stopDate = DateUtils.parseStringToDateYYMMDDHHmmss("2022-06-26 17:03:33");
    Date dateToThree = DateUtils.addMonthNumbersOfDate(startDate, 3);
    long compare = stopDate.getTime() - dateToThree.getTime();
